...
export function handleNewAutomatedVault_1(event: NewAutomatedVaultEvent): void {
  log.info('[GenericLpVaultFactory_1] Handle NewAutomatedVault tx: {}', [
    event.transaction.hash.toHexString()
  ]);
  let newAutomatedVaultCreatedAt = ethTxs.createEthTxFromEvent(event);

  let factory = factories.getOrCreateFactoryIfNeeded(
    event.address,
  ...
  strategies.createStrategiesFrom(lpVaultV2, FALSE, newAutomatedVaultCreatedAt);
}

export function handleNewAutomatedVault_2(
  event: NewAutomatedVault1Event
): void {
  log.info('[GenericLpVaultFactory_2] Handle NewAutomatedVault tx: {}', [
    event.transaction.hash.toHexString()
  ]);
  let newAutomatedVaultCreatedAt = ethTxs.createEthTxFromEvent(event);
...
export function handleNewAutomatedVault_3(
  event: NewAutomatedVault2Event
): void {
  log.info('[GenericLpVaultFactory_3] Handle NewAutomatedVault tx: {}', [
    event.transaction.hash.toHexString()
  ]);
  let newAutomatedVaultCreatedAt = ethTxs.createEthTxFromEvent(event);
