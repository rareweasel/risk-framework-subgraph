...
export function handleScoreSet(event: ScoreSetEvent): void {
  log.info("[RiskFramework] Handle ScoreSet tx: {}", [event.transaction.hash.toHexString()]);
...
export function handleTagSet(event: TagSetEvent): void {
  log.info("[RiskFramework] Handle TagSet tx: {}", [event.transaction.hash.toHexString()]);
  let tagSetAt = ethTxs.createEthTxFromEvent(event);
  let target = targets.getTarget(
    event.params.target,
    event.params._network,
  );
  tags.createTagUpdate(
    target,
...
export function handleTagRemoved(event: TagRemovedEvent): void {
  log.info("[RiskFramework] Handle TagRemoved tx: {}", [event.transaction.hash.toHexString()]);
